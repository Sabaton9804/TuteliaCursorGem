/**
 * Elimina por completo el caso 2026-1046 de Tutelia (BD + Storage).
 * Uso: npx tsx scripts/delete-case-1046.mts
 */
...
const { data: caseRow } = await admin
  .from('cases')
  .select('id, radicado, claimant, defendant')
  .eq('id', CASE_ID)
  .maybeSingle();

if (!caseRow?.id) {
  console.log('El caso 1046 ya no existe en Tutelia.');
  process.exit(0);
}

console.log('Eliminando:', caseRow);
...
console.log(check ? 'ERROR: el caso sigue en BD' : 'OK: caso 1046 eliminado de Tutelia.');
