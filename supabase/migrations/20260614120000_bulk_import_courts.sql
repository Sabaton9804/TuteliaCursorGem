-- Fase E: importación masiva de despachos (CSV / JSON batch).
-- depends on: 20260613130000_courts_platform_fields.sql (status, CUI)
-- depends on: 20260529120000_judicial_process_platform_phase1.sql (catálogos)

-- ---------------------------------------------------------------------------
-- Índice único por CUI (12 dígitos compuesto) para upsert idempotente
-- ---------------------------------------------------------------------------

create unique index if not exists courts_cui_composite_unique_idx
  on public.courts (
    dane_code,
    entity_code,
    specialty_code,
  ...
  where dane_code is not null
    ...
    and despacho_number is not null;

-- ---------------------------------------------------------------------------
-- Autorización: platform admin, service_role (CLI) o postgres (migraciones)
-- ---------------------------------------------------------------------------
...
comment on function public.bulk_import_is_trusted_caller () is
  'True para RPC bulk_upsert_courts: admin plataforma, service_role o conexión postgres.';

-- ---------------------------------------------------------------------------
-- bulk_upsert_courts — lote JSON [{ name, dane_code, … }]
-- ---------------------------------------------------------------------------
...
begin
  if not public.bulk_import_is_trusted_caller () then
    raise exception 'Solo administrador de plataforma o service_role pueden importar despachos';
  end if;
...
comment on function public.bulk_upsert_courts (jsonb) is
  'Upsert masivo de courts por id o CUI compuesto. Fase E — CSV/CLI y consola plataforma.';

revoke all on function public.bulk_upsert_courts (jsonb) from public;
grant execute on function public.bulk_upsert_courts (jsonb) to authenticated;
grant execute on function public.bulk_upsert_courts (jsonb) to service_role;
