...
- **Prompt:** versión `PIECE_AI_PROMPT_VERSION = 'v2.1'` en `src/lib/piece-ai-analysis.ts`.
- **Ramas de prompt:**
  - **CGP auto v2** (`cgp_auto_v2`): procesos `civil_*` o `catalog_metadata.tipo_registro = civil` y pieza que parece auto/providencia. Salida operativa para secretaría de un juzgado civil (CGP nacional; el membrete es del despacho). **Contestación verbal: art. 369 CGP** (20 días hábiles), alineado con `civil-business-days.ts`.
